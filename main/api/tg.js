...
bot.on(message('photo'), async (ctx) => {
    ...
    try {
        ...
        await requestState.updateOne({ status: 'completed' }),
        await ctx.deleteMessage(thinkingMsg.message_id).catch(() => { }),
        await ctx.reply(claudeAnswer).then(() => console.log('📤 Image response sent to user'))
    } catch (error) {
        ...
        await requestState.updateOne({ status: 'failed', error: error.message }),
        await ctx.deleteMessage(thinkingMsg.message_id).catch(() => { }),
...
bot.on(message('document'), async (ctx) => {
    ...
    try {
        ...
        // Store all documents in conversation history with caching
        const freshUser = await getUser(userId);
        const fileSizeInMB = fileBuffer.length / (1024 * 1024);

        // Store document in conversation history regardless of size (caching will handle efficiency)
        const newConvoHistory = [
            ...(freshUser.convoHistory || []),
            {
                role: "user",
        ...
        console.log(`📚 Document stored in conversation history with caching (${fileSizeInMB.toFixed(2)}MB)`);

        await updateUser(userId, { convoHistory: newConvoHistory });
        await requestState.updateOne({ status: 'completed' });
        await ctx.deleteMessage(thinkingMsg.message_id).catch(() => { });
        await ctx.reply(claudeAnswer).then(() => console.log('📤 Document response sent to user'));
